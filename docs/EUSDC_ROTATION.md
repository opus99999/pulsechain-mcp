...
`eusdc_rotation_recent_refresh` is the live signal-window refresher. It uses `syncPurpose: RECENT_SIGNAL_WINDOW`, a fixed requested window for resume tokens, newest-edge-first block scheduling, and WPLS/eUSDC anchor-first pool ordering. It builds a deterministic public task plan before scanning: shared WPLS/eUSDC anchor tip, required candidate tips, required 24-hour signal-window backfill, and optional diagnostic backfill. Each task owns one explicit block range, so completing a range always advances to the next range, pool, candidate, or phase. It writes only public market-history records and public checkpoint metadata, calls Piteas zero times, creates no approval or proposal, and performs no signing or broadcast.
...
History sync and recent refresh are bounded and resumable. The historical sync input accepts `maximumRuntimeMs` (default `180000`, capped below the Codex host tool timeout), optional `candidateIds`, optional `maximumPoolsPerRun`, optional `resumeToken`, and `syncPurpose` (`HISTORICAL_BACKFILL` by default). Recent refresh accepts `lookbackMinutes`, `tipRefreshMinutes`, `candidateIds`, `maximumRuntimeMs`, `maximumBlocksPerChunk`, `maximumPoolsPerRun`, `resumeToken`, `forceRecentBlockRecheck`, and `stopWhenAnyCandidateReady` (`true` by default). If the runtime budget is nearly exhausted, the tool returns `PARTIAL_PROGRESS`, releases `market-history.lock`, writes any idempotently merged public records, and persists a public checkpoint at:

```text
<resolved history directory>/sync-checkpoint.json
```

The checkpoint contains only public metadata: schema version, sync purpose, requested window, resolved block window, candidate set, required pool set, store path, chain id, deterministic task plan, plan fingerprint, pinned plan-tip block, authoritative current task index, completed task ids, completed block ranges, the pre-run store fingerprint, progress fingerprint, and update time. Redundant candidate/pool/source-cursor fields are diagnostic only and are rejected with `CHECKPOINT_STATE_INCONSISTENT` when they disagree with the authoritative cursor. A later call with the returned `resumeToken` resumes from the checkpoint instead of restarting completed work. A seven-day historical token is rejected for a 24-hour recent refresh, and a recent token for a different fixed end block/window returns `CHECKPOINT_WINDOW_MISMATCH`. There is no hidden continuation after the tool response ends.

Recent refresh pins the chain-tip block used to build one plan. Resumes do not move that finish line; after the plan completes, a later refresh can build a new plan for a newer tip and scan only the delta. Resume tokens bind the requested window, sync purpose, plan fingerprint, pinned tip block, current task index, completed task ids, and store fingerprint. They are generated only after checkpoint advancement is persisted, do not change merely because `updatedAt` changed, and do change when a task, range, phase, or cursor advances.

Recent refresh checkpoint progress is block-range based, not record-count based. A successful empty, duplicate-only, no-price, quiet-market, or out-of-retention range is still persisted as completed coverage and advances the authoritative current task index. Returned progress includes `planFingerprint`, `planTipBlock`, `currentPhase`, `currentTaskId`, `currentTaskIndex`, task counts, `nextRange`, `tasksAdvancedThisCall`, `rangesAdvancedThisCall`, `blocksScannedThisCall`, `sourceCallsThisCall`, zero-log and duplicate-only range counts, tip lag before/after, and progress fingerprints. `CHECKPOINT_STALLED` means pending work exists, no source request was made, the scheduler selected no valid next task or repeated a completed task, and no source error explains the stop. A complete plan returns `COMPLETE`, runtime expiry after real progress returns `PARTIAL_PROGRESS`, and a source failure returns `SOURCE_ERROR`.

Recent-refresh throughput is optimized without changing source proof. Live profiling of schema-3 refresh showed the main cost was not only block-range size: each 500-block range rebuilt the full record-key set, merged the full store, serialized and atomically rewrote the full store, reread/fingerprinted it, and wrote checkpoint state. Duplicate-only ranges could therefore consume tens of seconds while adding no records. Schema-4 refresh stages completed public ranges in memory, fetches each unique block header once per call with bounded concurrency, merges all staged records once, writes the public store once, then writes the checkpoint once with the resulting store fingerprint.

The commit order is deliberate: source ranges and records are processed in memory, the append-only store is built and atomically written, the resulting store fingerprint is computed, and only then is the checkpoint advanced for ranges represented in the written store or proven as zero-log/duplicate-only coverage. A crash before the store write leaves the checkpoint unadvanced. A crash after the store write but before checkpoint write may replay a range, and record-key deduplication makes that replay safe. The checkpoint is never intentionally ahead of the store.

`eusdc_rotation_recent_refresh` returns public throughput telemetry under `performance`: total elapsed time, lock wait, planning, source reads, log decode, block-header reads/cache hits, anchor joins, store read/merge/write, checkpoint write, finalization, task/range/block counts, source call counts, blocks/records per second, and bounded per-task timing diagnostics. The performance data contains only public chain/store metadata and no calldata, wallet data, keys, approvals, proposals, quotes, or transactions.

`eusdc_rotation_refresh_performance` is a read-only inspection tool for the active public checkpoint. It reports current plan schema, cursor, completed and remaining ranges, p50/p95 range timings from the last refresh response, block-header cache statistics, store timing, projected bounded calls to complete required PLS history and all five candidates, and the last performance fingerprint. It does not call Piteas, acquire the public writer lock, write wallet files, or perform any chain write.

Older recent-refresh checkpoints without a deterministic task plan are not silently reused. They are archived as `sync-checkpoint.stalled-<UTC timestamp>.json`, exact completed ranges are reused only when they match the newly generated plan identity, and the response reports `checkpointReplanned`, the archive path, old progress fingerprint, new plan fingerprint, reused range count, and scheduled-again range count.

When `stopWhenAnyCandidateReady=true`, recent refresh may stop after a task completes and history status proves one candidate is independently ready for live scanning. It returns `READY_CANDIDATE_FOUND` and the candidate id, releases the lock, and still does not run a rotation scan or create any trade authorization. Callers that want full diagnostic backfill can set `stopWhenAnyCandidateReady=false`.
...
The entry signal is deliberately narrow. A candidate must have complete current 24-hour source history or qualified sparse-event history, show a verified decline from a rolling local reference, show an actual-trade rebound with volume confirmation, avoid a new lower low afterward, avoid severe continuing downtrend, pass liquidity and route checks, and show historical evidence that the dynamic gross target is plausible. Candidate history readiness is scoped per candidate: an incomplete PLSX, INC, pHEX, or PRVX row remains visible diagnostically but does not make a ready PLS row return global `INSUFFICIENT_HISTORY`. If no candidate is independently ready, the scan returns `INSUFFICIENT_HISTORY`. If one or more candidates are ready but none satisfies the current entry signal, the scan returns `HOLD_EUSDC` or `INSUFFICIENT_EVIDENCE`. Only an eligible ready candidate may be selected, ties among ready eligible candidates return `HOLD_EUSDC`, and source failures become global only when every otherwise selectable candidate is blocked by a required shared/source failure.

Readiness details are reported in `candidateReadiness`, `historyReady`, `readyCandidateIds`, `incompleteCandidateIds`, `selectionCandidateIds`, `selectionScope: "READY_CANDIDATES_ONLY"`, and `historyDecisionReason`; these fields are included in the scan fingerprint. Current 24-hour signal readiness is separated from optional seven-day statistics. A non-blocking seven-day or leading-boundary diagnostic gap may produce a readiness warning without excluding an otherwise current-window-ready candidate, while required current-window pool gaps, stale latest trades, unsupported sources, source truncation, missing route evidence, or missing entry metrics still block that candidate.
